...
export const GameSquare = ({ square, index, active, activeGroup, onClick, errored }) => {
  ...
  return (
    <div 
      className={`${presolved ? 'prefilled-square': ''} game-square game-square-${index} ${mode === 'errors' && answer !== value && Number(value) !== 0 ? "red-text" : ""} ${active ? 'active' : ''} ${status === 'solved'? status: ''} ${status ==='cpu-solved'? 'cpu-solved': ''} ${activeGroup ? 'active-group' : ''} ${errored ? 'game-error': ''}`} 
